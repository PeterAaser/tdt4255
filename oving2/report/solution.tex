...
In the sequence diagram in fig:HAZARD we show an unhandled hazard. When a load word is issued the following instruction which depends on the destination register cannot read from the data memory output, shown as a red arrow.
...
To stop this from happening we must stall the first of the add instructions as shown in fig:HANDLED.
...
The second hazard we face occurs when a branch or jump occurs.
