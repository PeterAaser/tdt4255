...
If either operand in execute stage reads from the destination register of the operation currently at the mem stage, the forwarder \textbf{TODO:}forwards this data in place of the register data.
...
In addition to\textbf{TODO:} forwarding data for the execute stage the forwarder has one more task:
...
\textbf{TODO:} When the forwarder is unable to provide the data needed in the execute stage we have a data hazard. In our design the only source of data hazards are load words where the next operation reads data from the load word instructions destination register. 
